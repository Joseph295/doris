...
1. **原理**——按系统设计面试的思路展开，严禁泛泛而谈，必须回答三连问：
   - **遇到了什么问题？**——把该机制要解决的问题场景具体化（数据规模、并发、故障模式等约束）；
   - **有哪些候选方案，各有什么优劣？**——列出业界/理论上的可选方案并逐一分析权衡；
   - **Doris 最终怎么考量和解决的？**——落到 Doris 的实际选择，解释在其约束下为何这样取舍。
2. **源码走读**——沿执行路径解析真实代码，关键函数给出调用链图。严格分清主次：
   - 显而易见的代码高度概括、一笔带过；不易理解的代码详细逐段解释；
   - **重点挖掘易错点和 tricky 点**（隐蔽的约定、反直觉的实现、边界条件、并发陷阱、容易改错的地方），这些地方要给出详细解释：为什么这么写、错写会发生什么。
3. **双模式对比**——存算一体与存算分离在该环节的路径差异；涉及 MetaService 的深入 `cloud/src/`。
4. **动手实验**——源码级实操（编译目标、断点/日志位置、UT 运行命令、场景构造），实验设计紧扣两个目的：
   - **验证核心点**——通过断点/日志/UT 亲眼观察本章核心机制的运转；
   - **踩一遍易错点**——主动构造出易错场景（改错一行代码看 UT 如何失败、触发边界条件看行为），把"坑"变成肌肉记忆。
...
- "所以然"优先：每个机制按"问题 → 候选方案与权衡 → 最终取舍"的系统设计面试思路展开，再看实现。
- 详略有致：篇幅向"难懂的、易错的、tricky 的"倾斜，显而易见的内容高度压缩。
