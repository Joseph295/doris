...
| 部分 | 一句话主题 | 状态 |
|---|---|---|
| 第一部分：全局架构与设计哲学 | 建立 FE / BE / MetaService 的职责边界与存算一体、存算分离两种形态的全局心智模型 | 写作中 |
...
**代码引用格式**：全系列中出现的 `路径:行号` 引用均基于 master 分支、commit `0cd9e89cbb`（`git rev-parse --short HEAD` 的输出）。后续代码演进可能导致具体行号漂移，但路径和函数名的核实原则不变——写作时先在当前代码中确认存在，再落笔引用。
