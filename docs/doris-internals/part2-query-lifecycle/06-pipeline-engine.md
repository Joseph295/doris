...
Doris 这套引擎经历过一次重要演进：早期 pipeline 与后来的 **PipelineX** 长期并存，PipelineX 把算子的"无状态逻辑"（`OperatorX`，全实例共享一份）和"每实例运行时状态"（`LocalState`，每 task 一份）彻底拆开，解决了老 pipeline 里每实例重复构建算子、共享哈希表（broadcast join）难做、runtime filter 难以跨实例协调等问题。如今 PipelineX 已合入成为唯一实现——你在 `be/src/exec/pipeline/pipeline.h:54` 的注释里还能看到 `Add operators for pipelineX` 这样的历史印记，而所有算子类名都带 `X` 后缀（`OperatorX`、`DataSinkOperatorXBase`，见 `be/src/exec/operator/operator.h:812`、`:576`）就是这次演进的化石。
...
> **进阶：阻塞型 task 走单独的调度器。** 有些 task 本身注定要长时间阻塞（比如某些 sink 的收尾）。`HybridTaskScheduler`（`be/src/exec/pipeline/task_scheduler.h:85`）的 `submit()`（`be/src/exec/pipeline/task_scheduler.cpp:170`）会先问 `task->is_blockable()`，是的话丢进一个独立的 `_blocking_scheduler`（`:113`）线程池，不占用主调度线程池的名额——这样阻塞型 task 不会挤占非阻塞 task 的 CPU。
