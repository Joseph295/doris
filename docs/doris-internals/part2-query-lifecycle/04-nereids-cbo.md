...
- **`Memo`**（`fe/fe-core/src/main/java/org/apache/doris/nereids/memo/Memo.java:74`）：所有 Group 的容器。插入计划走 `copyIn`（`fe/fe-core/src/main/java/org/apache/doris/nereids/memo/Memo.java:264`），它用一张 `groupExpressions` 哈希表做**去重**：新建的 `GroupExpression` 如果结构上等于已有的（`fe/fe-core/src/main/java/org/apache/doris/nereids/memo/Memo.java:465`），就直接复用、不新建 Group（`:461`~`:468`）。**这正是"分组去重"落地的那一行**——两条不同规则若推导出结构相同的子计划，会被折叠进同一个 Group，代价只算一次。
...
- **确认收集是否真的完成。** `SHOW ANALYZE`（`fe/fe-sql-parser/src/main/antlr4/org/apache/doris/nereids/DorisParser.g4:894` 一带）看任务状态；`SHOW TABLE STATS` 看行数和最近更新时间，判断是否过期。
